...
// Matches now share one Chess960 position per (bracket branch, round) tier
// — e.g. every Winners Round 1 match uses the same position — so the picker
// shows one entry per tier, not one per match. t.bracket.matches is already
// in construction order (all Winners rounds, then all Losers rounds, then
// Grand Final), so preserving first-seen order here is enough to get a
// sensible left-to-right/top-to-bottom ordering without re-deriving it.
function groupMatchesByRoundTier(matches) {
  const tiers = new Map();
  matches.forEach((m) => {
    ...
    tiers.get(key).matchCount += 1;
  });
  return [...tiers.values()];
}

function tierLabel(tier) {
  return tier.bracket === "GF"
    ? `Grand Final${tier.round > 1 ? " (Reset)" : ""}`
    : `${bracketLabel(tier.bracket)} · Round ${tier.round}`;
}

// Bracket matches don't share a synchronous "round" the way Swiss/round-robin
// rounds do, but every match within one (bracket branch, round) tier does
// share a single Chess960 position — rolled the moment the first match in
// that tier activates (see tournamentService.js's activateMatch). So instead
// of the round picker used for Swiss/round-robin, brackets get a tier picker
// here.
function BracketMatchPicker({ tiers, selectedKey, onSelect }) {
  return (
    <div
      style={{
        display: "flex",
      ...
      {tiers.map((tier) => {
        ...
        return (
          ...
          >
            ...
            <div>
              {tier.matchCount} match{tier.matchCount === 1 ? "" : "es"}
            </div>
          </button>
        );
      })}
...
export default function Chess960() {
  ...
  // Elimination brackets: one position per (bracket branch, round) tier.
  const matchHistory = isBracket
    ? (t.bracket.matches || []).filter((m) => m.chess960)
    : [];
  const tiers = isBracket ? groupMatchesByRoundTier(matchHistory) : [];
  ...
  useEffect(() => {
    ...
    setSelectedTierKey((prev) =>
      prev && tiers.some((tier) => tier.key === prev)
        ? prev
        : tiers[tiers.length - 1].key,
    );
  }, [tiers.length]);
  ...
  const selectedTier = isBracket
    ? tiers.find((tier) => tier.key === selectedTierKey)
    : null;

  const current = isBracket
    ? selectedTier?.chess960 ?? null
  ...
  const emptyStateHint = isBracket
    ? "Each round of the bracket gets its own random position the moment its first match is ready — check back once the bracket starts filling in."
  ...
  const footerHint = isBracket
    ? selectedTier
      ? `Every board across ${selectedTier.matchCount} match${
          selectedTier.matchCount === 1 ? "" : "es"
        } in ${tierLabel(
          selectedTier,
        )} starts from this position — set boards up accordingly before play begins.`
      : "Every board in this round of the bracket starts from this position — set boards up accordingly before play begins."
